...
def df_to_ocrbox(df: pd.DataFrame):
    ...
    for i in range(len(df)):
        ...
        ocr_boxes.append(
            aocr.OCRBox(
                [text], aocr.otypes.BBox(*boxes[i]), [df.id[i]], [df.confidence.iloc[i]]
            )
        )
...
class TextractEngine:
    def __init__(self, bucket_name: str, aws_profile: str = "default") -> None:
    ...
    def _extract_tables(self, doc: trp.Document):
        ...
        for p, page in enumerate(doc.pages):
            ...
            rdf = pd.DataFrame(
                zip(reads, rids, rtidx, rconfs, rx1, ry1, rx2, ry2),
                columns=[
                    "text",
                    "id",
                    "table_num",
            ...
            pages.append(aocr.OCRResult(rdf, tables, tconfs, tboxes, ridxs))
        return pages
    ...
    def _initiate_image_request(self, name: str, resource: Image.Image) -> Any:
        ...
        return call_textract(
            url, features=[Textract_Features.TABLES], boto3_textract_client=self._tx
        )
