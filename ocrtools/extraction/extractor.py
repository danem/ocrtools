    ...
    def extract(
        ...
        for group in self._groups:
            ...
            for extractor in group.extractors:
                targets = [
                    box
                    for box in result.reads
                    if otypes.bbox_overlaps(extractor.box, box.box)
                ]
        ...
        for img, group, extractor, boxes in mappings:
            ...
            for value, box in values:
                ...
                nbox = box.box.translate(-group.box.x, -group.box.y)
                lbox = oocr.OCRBox([f"{extractor.name}: {value}"], nbox, [0], [1])
                logs[img].append(lbox)

        if logger:
            for img, boxes in logs.items():
                logger.info(page, img, [self, boxes])

        return results


def make_extractor(
    merger: oocr.IOCRBoxMerger, fn: Callable[[str], Any]
) -> IExtractionFn:
...
def get_date(
    tagger: otag.INERTagger, merger: oocr.IOCRBoxMerger = oocr.DefaultMerger
) -> IExtractionFn:
...
def get_named_entity(
    tagger: otag.INERTagger,
    merger: oocr.IOCRBoxMerger = oocr.DefaultMerger,
    confidence: float = 0.7,
) -> IExtractionFn:
    return make_extractor(
        merger, lambda t: otag.extract_named_entities(tagger, t, confidence=confidence)
    )
...
def match(
    pattern: re.Pattern, merger: oocr.IOCRBoxMerger = oocr.DefaultMerger
) -> IExtractionFn:
    def ret(boxes: List[oocr.OCRBox]):
        boxes = merger(boxes)
        results = []
