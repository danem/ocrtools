...
class BBox:
    ...
    @staticmethod
    def from_xywh(x: float = 0, y: float = 0, w: float = 1, h: float = 1):
    ...
    def transform(self, mat: np.array):
        ...
        return BBox.from_xyxy(tl[0], tl[1], br[0], br[1])
    ...
    def as_tuple(self, format="xyxy", width=1, height=1):
        ...
        if format == "xyxy":
            return (
                self._points[0] * width,
                self._points[1] * height,
                self._points[2] * width,
                self._points[3] * height,
            )
        elif format == "xywh":
            return (
                self._points[0] * width,
